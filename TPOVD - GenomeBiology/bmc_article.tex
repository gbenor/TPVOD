...
A good classifier performance is characterized by high TPR and TNR (closer to 1) and low FNR and FPR (closer to 0). Moreover, TPR is almost equal to TNR (same rate of true detection) and FPR is almost equal to FNR (same rate of miss-detection), indicating that all  8 classifiers (corresponding to each dataset) are balanced.
PPV and  NPV describe the precision of the classifier, which is the proportion of the true predictions from all predictions. The value of these measures ranges from 0 (worst) to 1 (ideal). For all datasets, the classifiers achieve rates of 0.83-0.95 indicating that these classifiers are precise.
